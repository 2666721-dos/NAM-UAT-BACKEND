    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "この場合、符号の違い（プラス／マイナス）は無視し、絶対値の差が Reference で定義された許容範囲（例：10％以内）に収まるかどうかを基準に判断してください。",
                "絶対値の差が許容範囲内であれば「概ね同程度」「整合」と判断し、符号の違いのみを理由に不整合と判定してはいけません。",
                "このルールが発動している場合、方向性一致ルール（上昇／下落の一致判定）は適用しないでください。必ずこのルールを優先します。",
