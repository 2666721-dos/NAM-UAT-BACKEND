    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "【文意一致ルール（最上位判定）】",
                "Org_Text と Input の 主題（テーマ）および主語（対象・主体）が一致しない場合、Focus に基づく検証を行ってはいけません。",
                "この場合は 空の文字列（""）を返してください。",
                "特に、主語に含まれる国名が異なる場合は必ず空文字列を返すことを明確にします。",
                "方向性（上昇／下落など）が異なるだけの場合は対象外ではなく、通常どおり比較を行います。",
                "例：",
                "Org_Text：日本国債の利回りが上昇",
                "Input：フランス国債の利回りが上昇",
                "⇒ 主題・主語が異なるため空の文字列を返す（対象外）",
                "Org_Text：日本国債の利回りが上昇",
                "Input：日本国債の利回りが下落",
                "⇒ 主題・主語が一致しているため比較対象（方向性のみ異なる）",
                "このルールは全ての判定よりも優先されます。主題・主語が異なる場合、他のいかなる比較・判定も行ってはいけません。",
