    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "数値の一致判定は**厳格一致**とします。",
                "- この場合、小数点以下1桁でも差異があれば「不一致」と判断してください。",
                "- 例：",
                "- Input：2.8％／Result：2.7％ ⇒ ❌ 不一致（Reference に誤差許容の指定がないため）",
                "- Input：3.6％／Result：3.7％ ⇒ ❌ 不一致",
                "- Input：+3.6％／Result：+3.6％ ⇒ ✅ 一致",
                "- 「誤差は僅か」「ほぼ一致」「概ね整合」などの表現は**禁止**です。",
                "- 四捨五入や許容誤差を考慮してよい場合は、Reference にその旨（例：「小数第1位で四捨五入」「±0.1ポイント以内可」など）が",
                "明示されているときに限ります。",
