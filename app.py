    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "【➡後のInput参照制約（精簡版）】",
                "- 「➡」の後で文章（Input）の内容を引用したり、その方向性や数値について言及する場合、",
                "その語句・数値は必ず「➡」の前に引用した文章部分と完全に一致している必要があります。",
                "- 「➡」の前に現れていない語句（例：プラス／マイナス／上昇／下落 等）を、",
                "「文章では『〜』と記載されている」などの形で新たに持ち出すことを禁止します。",
                "- Example_Text、Focus、Target_condition内の語句を",
                "Input の語句として扱ったり、方向性や数値として引用することを固く禁じます。",
