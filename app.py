    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "【前処理ルール（必ず最初に実行・無条件遵守）】",
                "まず、Org_Text と Input の両方について、文頭に「上記の環境のもと、」「上記の投資環境のもと、」が含まれているか確認する。",
                "上記前置句が文頭に存在する場合、無条件で削除し、削除後のテキストをそれぞれ「処理後 Org_Text」「処理後 Input」として定義する。",
                "この削除操作は「文意判定の前に絶対に実行」するものとし、省略またはスキップした場合の判定結果はすべて無効とする。",
                "以降の「削除確認工程」「原文一致性チェック」「数値比較」では、「処理後 Org_Text」「処理後 Input」のみを使用し、処理前のテキストを参照しない。",
                "【削除確認工程（必ず実行・中止条件付き）】",
                "「処理後 Org_Text」と「処理後 Input」の文頭を確認し、「上記の環境のもと、」「上記の投資環境のもと、」が残存しているか検証する。",
                "いずれか一方でも前置句が残存している場合は、即座に空文字（"") を返し、以降のすべての処理を中止する。",
                "両方の前置句が正常に削除されている場合のみ、次の「原文一致性チェック」に進む。",

                "【前提条件：原文一致性チェック（必ず最初に実行）】",
                "Org_Text と Input を比較し、共通の前置句（例：「上記の環境のもと」「当月の市場環境を踏まえ」など）を除いた後の主要部分（主語・要因・述語など）の語彙が一致しない場合は、ただちに空文字（\"\")を返し、以降の判定を行わないでください。",
                "特に、「保有する債券」「株式」「為替」「利回り」「金利」「物価」「セクター」など、要因や対象資産を示す語が異なる場合は、文意が異なるとみなし、空文字を返してください。",
                "この判定は Result に依存してはいけません。文意と語彙の類似度のみで行ってください。",
                "Input が Org_Text と同じ文型でも、異なる資産・要因・経済要素を含む場合（例：Org_Text＝債券、Input＝米国債利回り）は『異なる内容』として扱い、空文字を返してください。",
                "一致度の判定は厳密に行い、わずかな主語・要因の違いでも一致とみなしてはいけません。曖昧な場合も空文字を返してください。",
                "さらに、Org_Text と Input の両方に出現する主語・要因部分（「〜が」「〜は」「〜により」などの直前の名詞）を抽出し、それらの主要名詞が異なる場合は必ず空文字を返してください。",
                "特に、Org_Text に含まれない具体的名詞（例：米国債、株式、為替、金利、物価、セクター、国名、企業名など）が Input に含まれる場合は即座に空文字を返してください。",
                "句構造が類似していても、主語または要因の名詞が異なる場合は一致とみなしてはいけません。",
                ...
                "【補足パラメータの説明と使用ルール】",
                "① Org_Text：Inputに対応する原文の一部を示す文章。数値部分は過去の値である可能性があるため、Org_Text内の数値は比較対象としないでください。文意のみ参考にしてください。",
                "② Focus：Org_Textの中で特に注目すべき語句または数値。複数のチェックルールが同一のOrg_Textを持つ場合でも、Focusが異なれば注目点を変えて比較を行ってください。つまり、Focusが指す部分を重点的に評価対象とします。",
                "③ Reference：Resultからデータを検索・抽出するための説明文またはルール。Referenceの指示に従い、Result中の該当データ（数値や語句）を取得し、その値や意味をInput内の該当箇所と比較します。",
                " 例：Resultに「+3.49%」、Inputに「+2.88%」が存在する場合 ⇒ 不整合と判断する。",
                " 例：Resultに「+3.49%」、Inputに「下落」など逆方向の語が存在する場合 ⇒ 不整合と判断する。",
                "④ Target_condition：最終判定時に使用する条件を示します。抽出・比較後の値をこの条件に照らして評価します。",
                " 例1：『米ドル建て投資適格社債の純資産比は80％以上』 ⇒ 『高位』と判断。",
                " 例2：『-0.48%』 ⇒ 『マイナス』と判断。",
                "Target_conditionは、最終的な意味判定または方向性判断に使用してください。",
