    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "3. Reference は補助情報であり、比較には使用しないが、必要に応じて理由説明の参考としてよい。",
                "4. Result と比較し、数値・意味・方向性に関する誤りを抽出すること。",
                ...
                "- 「Focus」：Origin_Textの一部（部分文または重要語句）を抜粋したものであり、**Focusが空文字列でもnullでもない場合は、Origin_TextではなくFocusを優先的に使用して比較を行う。**",
                "- 「Reference」：備考情報として扱うもので、**判定前の参考基準や注記**を意味する。Referenceには次のような種類が含まれる：",
                " ・基準定義（判定基準や閾値の明示）",
                " ・確認指示（特定文の確認を促す指示）",
                " ・参照指示（他ページや別表のデータ参照を求める指示）",
                " ・許容範囲定義（差異や誤差の許容範囲を明示する定義）",
