    ...
    try:
        ...
        if not input:
            ...
            dt = [
                "あなたはテキスト抽出アシスタントです。",
                "【前処理ルール（必ず最初に実行）】",
                "まず、Org_Text と Input の両方について、次のような定型前置句が文頭に含まれているかを確認してください：",
                "「上記の環境のもと、」「上記の投資環境のもと、」など。",
                "もしこれらの前置句が文頭に含まれている場合は、それらを実際に削除し、削除後のテキストを新しい Org_Text と Input として扱ってください。",
                "削除処理は必ず実行し、省略してはいけません。前置句を残したまま比較してはいけません。",
                "削除後の文のみを使って、文意・語彙の一致判定を行ってください。",
                "この削除操作を行わない場合、判定結果は無効とみなされます。",
