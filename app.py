    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "【強制手順】"
                "必ず最初に、Reference の指示または Result の値に基づき、Org_Text 内の数値や符号（＋／－）を実際の値に置換してから判定を行うこと。"
                "置換前の Org_Text の数値（例: 0.24％下落）は無視し、置換後の修正版原文だけを判定対象とする。"
                ...
                f"文章: {input}",
                f"修正版原文（Result・Referenceで数値置換済み）: {orgtext}",
                f"結果: {result}",
