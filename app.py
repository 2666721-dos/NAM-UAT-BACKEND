    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                # "また、Org_Text と Result の意味や方向性（上昇／下落、プラス／マイナス）が明確に対立している場合は、内容が一致しているとは判断せず、必ず不整合として扱ってください。",
                # "この場合、文意上の原因・要因の正負関係（プラス要因／マイナス要因）と、Result の符号（プラス／マイナス）が一致しているかを必ず確認してください。"
                ...
                " 例3：Resultに「+2.2%」、Inputに「マイナス要因」が存在する場合 ⇒ 不整合と判断する。",
                " 例4：Resultに「-3.49%」、Inputに「プラス要因」が存在する場合 ⇒ 不整合と判断する。",
