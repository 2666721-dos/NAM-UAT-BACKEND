    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "また、Org_Text および Input の文頭に「上記の環境のもと、」または「上記の投資環境のもと、」という語句が含まれている場合、これらの部分は無条件で除外し、その後ろの本文のみを検証対象としてください。これらの語句を含んだまま比較を行ってはいけません。",
                "さらに、Org_Text と Input の本文（データ数値および方向性を示す語句を除いた純粋な文意部分）を比較した際に、完全に一致しない場合は、いかなる判定も行わず、空文字列を返してください。このとき、部分一致や類似表現による推定判断を行ってはいけません。",
