    ...
    try:
        ...
        if input:
            dt = [
                ...
                "◆【最優先ルール：Focus による文節固定】（この指示は最上位の優先度とする）",
                "- 判定処理は必ず最初に、Focus が含まれる文節（句読点「、」「。」で区切られる最小単位）を特定し、",
                "『この文節のみ』を最終的な評価対象としてください。",
                "- Focus が存在しない文節や、後続の別文節（例：「ファンドは…」など）は、内容が整合していても",
                "評価対象に含めてはいけません。",
                "- Reference が別の対象（例：ファンド値）を指示している場合でも、",
                "文節選択より優先されることはなく、Focus の属する文節以外へ判定範囲を拡大することは禁止します。",
                "- よって、Focus が指数の文節に属する場合、後半のファンド文節は",
                "引用・比較・抽出のいずれでも使用してはいけません。",
                "- この文節固定ルールは、全ルールの中で最も優先度が高く、",
                "Reference、方向性判断、数値比較、派生値計算など、すべてのルールより優先されます。",
                ...
                "- Focus所属的句节(标点符号“、”“。”或逗号分隔的最小单位)作为判定对象。",
                "- 禁止跨不包含Focus的其他句段进行判断。",
                "- 特别是像“A、B”这样多个句节并列的情况下，",
                "- 当Focus包含在A中时，绝对不能将整个B（例如，关于基金的描述）作为评价对象。",
                "- 即使Reference指示另一个值（例如基金值），",
                "- 禁止跳到Focus所属的句段和不相关的句段进行解释。",
