    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "【Target_condition 追加ルール（短句強制一致・特定ケース判定）】",
                "- Target_condition の内容が特定の短句に一致する場合、Input（文章）内にも対応する短句が存在していなければなりません。",
                "- 特に、Target_condition が「香港ドル/円の代替為替ヘッジ」である場合は、以下の語句が Input 内に存在するかどうかのみを基準として判定してください。",
                "- 以下の語句のいずれかが Input 内に存在する必要があります：",
                "● 「香港ドルによる代替ヘッジを含む」",
                "● 「（香港ドルによる代替ヘッジを含む）」",
                "- 上記の語句が Input 内にまったく存在しない場合、必ず「不整合」と判断してください。",
                "- この判定は数値比較や方向性判定とは独立して適用され、Result や他のルールに依存しません。",
                "- まとめ：",
                "  ● Target_condition = 「香港ドル/円の代替為替ヘッジ」",
                "  ● Input 内に「香港ドルによる代替ヘッジを含む」（括弧あり/なしのいずれか）が存在 → 整合",
                "  ● Input 内にまったく存在しない → 必ず不整合",
                "- この短句対応ルールは優先度が非常に高く、この条件に違反した場合は他の情報がすべて整合していても最終結論は「不整合」とします。",
            ...
            if _parsed_data:
                for once in _parsed_data:
                    ...
                    negative_patterns = [
                        ...
                        r"誤り",
                        r"不整合",
                        r"整合(?:していません|していない|されていません|されていない|が取れていません|性がありません)",
