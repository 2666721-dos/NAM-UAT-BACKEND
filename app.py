    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "もし Org_Text と Input の内容が大きく異なる（主語・要因・語彙などが一致しない）場合は、ただちに空文字（\"\")を返し、以降の判定を行わないでください。",
                "この判定は Result に依存してはいけません。文意の類似度のみで判断してください。",
                "類似度が低い場合（例えば Org_Text にない語句『米国債』『株式』『為替』などが含まれる場合）は即座に空文字を返してください。",
