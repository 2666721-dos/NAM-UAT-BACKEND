    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "【前提条件（Referenceなしの処理）】",
                "Reference が null、空文字、または未指定であっても、Org_Text に数値（%, 円, pt, bp, 年 など）が含まれる場合は通常どおり Result 参照・置換・比較を実施する。",
                "Org_Text に数値が一切含まれない場合のみ、参照・比較をスキップし、空文字 \"\" を返す（この場合は Result の月や値を推測しない）。",

                "【月固定と参照解釈（最優先）】",
                "Reference に『resultの(\\d+|[０-９]+|[一二三四五六七八九十]+)月の(.+?)を参照』等の指示がある場合：",
                " - 月は全角/漢数字を半角アラビアに正規化して列=該当月を特定する（例：８→8、八→8）。",
                " - 行名は空白差異を除去して正規化し（例：'月次 騰落率'≒'月次騰落率'）、行=該当項目を特定する。",
                " - 列×行で単一セル値を取得する。取得できない場合は空文字 \"\" を返し、他月への回退・推測は禁止。",

                "【表パース規則】",
                "Result の1行目は月ヘッダー（|４月|５月|…）。全角→半角に正規化して数値化（4,5,6,7,8…）。行名は空白・表記ゆれを正規化し、完全一致優先で突合する。",

                "【強制手順（置換→判定）】",
                "1) 最初に必ず、Reference の指示または上記で取得した Result の値に基づき、Org_Text 内の数値および符号（+/-）を実際の値に置換する。",
                "2) 置換対象は %, 円, pt, bp, 年 などの数値と符号（+/-）。符号と文言（上昇/下落）が矛盾する場合は、置換後の符号に合わせて文言も整合させる（+→上昇、-→下落）。",
                "3) Reference に丸めや小数点桁の指定（四捨五入/切り捨て/切り上げ/小数点桁）がある場合は、取得直後に加工してから Org_Text を置換する。",
                "4) 置換前の Org_Text（例: 0.24％下落）は無視し、置換後に新規生成した『修正版原文』のみを比較・判定の唯一の対象とする。",
                "5) 優先度は Reference ＞ Result ＞ Input ＞ Org_Text（未置換）とする。",

                "【比較要件】",
                "3. AIへの入力には、置換後の Origin_Text（＝修正版原文）を使用し、これを Input と比較して整合性を判断すること。",
                "3.1 修正版原文では、Result・Reference の条件に従って Org_Text 内の数値を実際に置換し、その置換値をもとに判定を行うこと（例：41.72%＋四捨五入→42%）。",
                "4. Result と比較し、数値・単位・符号・方向性（上昇/下落）・月/期間の不一致を抽出すること。",
                ...
                "- 「Org_Text」：過去の参考資料。**元の数値は使わず、Result/Reference に基づく置換値を用いる。**",
                "- 「Target_condition」：補助条件（符号・方向性・単位など）。数値自体は比較対象に含めない。",
                "- 「Focus」：Org_Text の注目領域または置換対象の手掛かり。",
                "- 「Result」：掲載データの確認元。",
                "- 「Reference」：取得先/加工条件/参照指定（例：月・項目指定）。",

                f"文章: {input}",
                f"Org_Text（未置換）: {orgtext}",
                "※上記の Org_Text は未置換です。上記ルールに従い、あなたが必ず置換後テキスト（修正版原文）を新規生成してください。",
                f"結果: {result}",
