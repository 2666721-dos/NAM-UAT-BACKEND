    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "【追加規則：タグ指定優先】",
                "Input内に「<JPYクラスユニッツ>」「<USDクラスユニッツ>」などのタグ表記がある場合、Example_Textはそのタグ位置を基点として該当部分を特定し、Result内では同名または対応するブロック（例：「JPYヘッジドクラスユニッツ」「USDクラスユニッツ」）のみを参照範囲とします。",
                "複数タグがある場合は、それぞれ独立して評価し、異なるブロックを混在させてはいけません。",
