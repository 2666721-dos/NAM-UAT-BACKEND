    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "【主語・主題一致確認プロトコル（最上位優先）】",
                "このプロトコルは、全ての判定ルールよりも優先して実行されなければなりません。",
                "次の手順を、必ず上から順に実行してください。途中で条件を満たさない場合は、直ちに空文字列（\"\"]）を返し、他のいかなる判定も行ってはいけません。",

                "Step 1：Org_Text と Input の主語（文の中心となる対象、例：日本国債・米国債・保有資産・為替など）が一致しているか確認します。",
                "Step 2：Org_Text と Input の主題（文全体が述べている事柄、例：金利上昇・価格下落・利息収入など）が一致しているか確認します。",
                "Step 3：主語または主題のいずれかが一致していない場合は、Focusや数値比較、方向性判定を一切行わず、空文字列（\"\"]）を返してください。",
                "Step 4：主語・主題の一致判定において、『上記の環境のもと』『プラス要因／マイナス要因』といった一般的な導入句や共通語句は、一致の根拠にしてはいけません。これらは意味比較の対象外とします。",
                "Step 5：Input文に『米国債』『外国債』『為替』『ドル』『円』『金利』などの語が含まれ、Org_Textにそれらが含まれていない場合、またはその逆の場合は、無条件で主語不一致と判断し、空文字列を返してください。",
                "Step 6：主語・主題の両方が一致している場合のみ、次のFocus使用ルールに進んでください。",

                "【Focus使用前提ルール】",
                "Focusは、Org_TextとInputの主語および主題がいずれも一致している場合にのみ参照対象とします。",
                "Focusは文意の補助的な注目点であり、主題・主語の一致確認を省略するために使用してはいけません。",
                "Focusに同一語句（例：プラス要因、マイナス要因）が含まれていても、主語・主題が一致していなければ一致判定・整合判定を行ってはいけません。",
                "Focusの評価は、主語・主題の一致確認が完了した後にのみ実施してください。",
                "このルールは、Reference、方向性判定、絶対値比較ルールよりも優先される上位ルールです。",
