    ...
    try:
        ...
        if not input:
            ...
            dt = [
                    ...
                    "- 「Target_condition」は参考情報として提示されているものであり、判断の補助として利用しますが、最終的な確認対象ではありません。",
                    "- 最終的な比較・検証は、「Result」の内容と「Input（文章）」中の該当箇所の整合性によって行ってください。",
                    "- つまり、「Result」に含まれる数値や語句が、文章（Input）中の実際の表現や数値と一致しているかを確認してください。",
                    "- 判断の最終基準は「Result」と「Input（文章）」の一致にあります。「Result」の語句や数値が文章中に正しく存在すれば、それが正しいと判断してください。「Origin_Text」や「Target_condition」の内容と一致していない場合でも、「Result」と一致していれば問題ありません。"
                    "- 「Origin_Text」はルールの定義上の参考文として扱い、直接的な比較対象ではありません。",
                    "- 「Focus」は、原文(Origin_Text)の中でも特に重要なキーワードまたは文節であり、確認時にはこの部分に特に注目してください。",
                    "- 「Focus」の内容は、現在のルールの「原文（Origin_Text）」に密接に関連する語句や文節を指定してください。",
                    "- もし「Focus」が比較的短い場合でも、必ず現在のルールで指定された「原文（Origin_Text）」との対応関係を重視して判断してください。文章全体の中で同様の語句が存在しても、Focusは原文に基づく内容として扱い、他の文脈に誤って一致させないようにしてください。",
                    "- 「Focus」に一致する語句が文章中に存在したとしても、その語句が含まれる全文が「原文（Origin_Text）」と完全には一致しない場合、そのテキストは抽出しないでください。",
                    "- 「Reference」は、特に数値や割合などの整合性を判断する際の参照基準となります。Focusと併せて、Referenceとの一致・整合性を重視して確認してください。",
