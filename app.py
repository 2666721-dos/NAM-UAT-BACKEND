    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "以下の条件に従って比較を行ってください：",
                "1. Reference の内容を解析し、必要に応じて Result から数値や条件を抽出・置換すること。",
                " (1) Reference に「四捨五入」「切り捨て」「切り上げ」が含まれる場合：Result の値をその通り処理して置換する。",
                " (2) Reference に「PAGE」「〇ヶ月」「騰落率」「比率」などが含まれる場合：Result の該当項目から数値を抽出して置換する。",
                " (3) Reference に「マイナスであることを確認」「プラスであることを確認」などが含まれる場合：抽出した数値がその条件を満たすかを確認する。",
                " (4) Reference に他ページや別表の参照が含まれる場合は、数値置換の対象外とし、理由説明に参考として用いる。",

                "2. Origin_Text に数値（%, 円, ptなど）が含まれる場合、それらを Result および Reference から得た正しい数値に置換して使用すること。",
                "  このとき、Focus はあくまで注目領域を特定するための手がかりとして利用し、",
                "  置換値そのものは Result・Reference の内容に基づいて決定すること。",

                "【強制手順（置換→判定）】",
                "1) 最初に必ず、Reference の指示または Result の値に基づき、Org_Text 内の数値および符号（+/-）を実際の値に置換すること。",
                "2) 置換対象は %, 円, pt, bp, 年 などの数値および符号（+/-）。符号と文言（上昇/下落）が矛盾する場合は、置換後の符号に合わせて文言も整合させる（+ → 上昇、- → 下落）。",
                "3) Reference が『resultの(\\d{1,2})月の(.+?)を参照』等の形式を含む場合は、Result を表として解釈し、列=該当月、行=該当項目から単一セル値を取得して用いる。",
                "4) 四捨五入/切り捨て/切り上げ/小数点桁などの加工指示が Reference にある場合、取得直後に加工してから Org_Text を置換する。",
                "5) 参照先セルが一意に特定できない/存在しない場合は、空文字列 \"\" を返す（理由は書かない）。",
                "6) 置換前の Org_Text（例: 0.24％下落）は無視し、置換後にあなたが新規生成した『修正版原文』のみを比較・判定の唯一の対象とする。",


                "3. AIへの入力には、置換後の Origin_Text（＝修正版原文）を使用し、これを Input と比較して整合性を判断すること。",
                "3.1 修正版原文では、Result・Reference の条件に従って Origin_Text 内の数値を実際に置換し、置換後の数値をもとに判定を行うこと。",
                "  たとえば Result に41.72%、Reference に四捨五入が指定されている場合、置換後の値は42%とする。",
                "4. Result と比較し、数値・意味・方向性の不一致を抽出すること。",
                ...
                "- 「Origin_Text」：過去の参考資料であり、文章全体の文意がInput内の一部と類似している場合に参照する。",
                "   ただし、**元の数値はそのまま使わず、Result・Reference に基づいて置換した値を使用する。**",
                "- 「Target_condition」：判断基準の補助情報であり、",
                "   **数値そのものではなく、符号（プラス／マイナス）・方向性・単位・程度などの条件を示す。**",
                "   したがって Target_condition の数値自体は比較対象に含めてはならない。",
                "- 「Focus」：Origin_Textの一部で、文中の注目点または置換対象を特定するために用いる。",
                "- 「Result」：実際に掲載された最新のデータであり、数値や方向性の確認元となる。",
                "- 「Reference」：Result の補足的説明や計算・条件指示を示す情報であり、",
                "   数値処理（四捨五入・切り捨て・平均・小数点指定など）や条件判断（プラス／マイナス確認、比較対象の指定など）を含む。",

                f"文章: {input}",
                f"Org_Text（未置換）: {orgtext}",
                "※上記の Org_Text は未置換です。以下の【強制手順】により、あなたが必ず置換後テキストを新規生成してください。",
                f"結果: {result}",
                f"判断根拠: {target_condition}",
                f"注目語句(Focus): {focus}",
                f"備考(Reference): {reference}"
            ]
