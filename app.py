    ...
    try:
        ...
        if not input:
            ...
            dt = [
                    ...
                    "- 「Input」は実際のレポート本文です。複数段落を含むことがあります。",
                    "- 「Origin_Text」は過去の原文や参考資料であり、現在の判断基準には使用しません。比較のための参考情報にとどめてください。",
                    "- 「Result」は最終的な比較・検証対象であり、実際の数値・記述が含まれます。",
                    "- 「Target_condition」はルール上の補助的な基準です（例：90%以上であれば「高位」とする）。",

                    "- 注意：「Org_Text」に数値や語句が含まれていても、それは過去情報です。「Result」として扱わないでください。",
                    "- 「Result」はJSON中で明示的に 'result' キーに指定された内容のみを指します。原文中の数値（Org_Text）は比較対象外です。",
                    "- 判定時、「Result」に書かれた数値・表現と「Input」本文の記載を照合してください。",
                    "- 「Org_Text」に似た内容が存在しても、それを「Result」と混同しないでください。",
                    "- 「Result」に含まれる数値が「Input」と一致する場合は、必ず正常（空文字列）としてください。",

                    "【判定の優先順位】",
                    "1. 最も重視するのは「Input（文章）」と「Result（結果）」の一致です。",
                    "2. 「Input」と「Result」の内容が一致している場合は、他の情報（Origin_Text, Target_condition）に相違があっても、誤りとは判定しません。",
                    "3. 「Input」と「Result」が不一致の場合のみ、誤りとして出力します。",
                    "4. 「Origin_Text」に記載された内容は参考情報に過ぎず、最終的な正誤判断には一切使用しません。",

                    "- つまり、「Result」に含まれる数値や語句が、「Input（文章）」中の記載と一致していれば、それを正しいと判断してください。",
                    "- 「Origin_Text」や「Target_condition」と一致していなくても、「Input」と「Result」が一致していれば問題ありません。",

                    "- 「Input」と「Result」に記載された数値や表現が一致する場合は、原文（Origin_Text）との相違があっても誤りとは判定しません。この場合は正常とみなし、出力を空文字列（\"\"）としてください。",

                    "- 「Focus」は「Input」または「Result」内で特に注目すべき語句やフレーズです。",
                    "- 「Focus」が短い場合でも、必ず「Input」文中での意味的関連性を重視し、誤った文脈に一致させないでください。",
                    "- 「Reference」は補足的な注釈情報であり、直接照合の対象にはしません。",

                    "あなたは日本の金融レポートを専門とするプロの校正者です。",
                    "例1（正しい場合）：",
                    "Input: 月末のヘッジコスト参考値（※）は、3.83％。",
                    "Result: ヘッジコスト|3.83%",
                    "Org_Text: 月末のヘッジコスト参考値（※）は、4.30％。",
                    "→ 「Input」と「Result」は数値が一致しているため、誤りではありません。出力は空文字列（\"\"）としてください。",

                    "例2（誤りの場合）：",
                    "Input: 月末のヘッジコスト参考値（※）は、3.83％。",
                    "Result: ヘッジコスト|4.30%",
                    "Org_Text: 月末のヘッジコスト参考値（※）は、3.83％。",
                    "→ 「Input」と「Result」が不一致（3.83 ≠ 4.30）のため、誤りと判断し、",
                    "[{'original': '月末のヘッジコスト参考値（※）は、3.83％。', 'reason': '文章中では3.83％であるが、結果では4.30％と記載されており一致していない。'}]",
                    "と出力してください。",

                    "以下の要約文（Input）をもとに、「結果（Result）」と文章との整合性を確認し、必要に応じて、類似した原文を抽出してください。"
