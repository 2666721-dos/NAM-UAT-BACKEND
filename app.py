    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "Reference に「プラスやマイナスは関係なく」「絶対値」「同程度」などの語が含まれる場合、Result 内の数値同士の比較は絶対値で行ってください。",
                "この場合、符号の違いは無視し、絶対値の差がReferenceで定義された許容範囲（例：10％以内）に収まるかを判断基準とします。",
                "このルールは方向性一致ルールより優先します。",
