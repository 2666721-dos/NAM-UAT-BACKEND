    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "2. Focus は Origin_Text の中で特に注目すべき部分（キーワードや句）であり、",
                "  数値や条件の置換を行う際は Focus と Reference の内容を優先して解析すること。",
                "  Origin_Text に数値（%, 円, ptなど）が含まれる場合、それらを Reference や Focus から得た正しい数値で置換して使用すること。",
                "  つまり、AIへの入力には置換後の Origin_Text（＝修正版原文）を使用すること。",

                "3. Focus 自体は文脈上の注目点として利用するが、比較対象は置換済みの Origin_Text 全体とする。",
                ...
                "- 「Origin_Text」：過去の参考資料であり、文全体の文意がInput内の一部と類似している場合に参照する。ただし、**元の数値はそのまま使わず、Focus・Referenceに基づき置換した値を使用する。**",
                "- 「Target_condition」：補助的な判定基準（例：80%以上なら「高位」とみなす、など）。",
                "- 「Focus」：Origin_Textの一部で、文中の注目点または置換対象を特定するために用いる。",
                "- 「Result」：実際に掲載された最新のデータであり、数値や方向性の確認元となる。",

                f"文章: {input}",
                f"修正版原文（Focus・Reference で数値置換済み）: {orgtext}",
                f"結果: {result}",
                f"判断根拠: {target_condition}",
                f"注目語句(Focus): {focus}",
                f"参照基準(Reference): {reference}"
            ]
