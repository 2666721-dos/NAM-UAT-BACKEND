    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "【置換の目的（補助）】",
                "Origin_Text に数値（%, 円, pt など）が含まれる場合、Reference/Result を用いて値と符号（+/-）を置換し、Input 内の該当記述の同定や差分抽出の補助に用いる。ただし、裁定は Result×Input のみで行う。",
                ...
                "- 1行目は「|」区切りで月（4月〜3月, 年度累積）を定義。",
                "- 以降の各行は行頭の指標名と値列が対応。",
                "- 年度累積は比較対象外。",
                "- Reference の「〜の行の〜の欄を参照します」という指示があれば、",
                "  その指標行 × 月列の交点の値のみを抽出する。",

                "【Reference の扱い（補助）】",
                "Reference の内容を解析し、必要に応じて Result から数値や条件を抽出・置換すること。",
                "(1) 「四捨五入」「切り捨て」「切り上げ」が含まれる場合：Result の値をその通り処理して置換する（補助目的）。",
                "(2) 「PAGE」「〇ヶ月」「騰落率」「比率」などが含まれる場合：Result の該当項目から数値を抽出する（補助目的）。",
                "(3) 「マイナスであることを確認」「プラスであることを確認」等は整合性チェックの補助に留め、最終裁定には使用しない。",
                "(4) 他ページ/別表参照は置換の対象外とし、必要に応じて説明に参考としてのみ用いる。",
                ...
                "例：Result に41.72%で Reference に四捨五入指定がある場合、説明時の比較値は42%だが、裁定根拠は Result（およびその指示に従った加工値）と Input の対比による。",
