    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "3) Input 側の方向性は、Input の文言のみから読み取る（例：「上昇」＝プラス方向、「下落／マイナス要因」＝マイナス方向）。Org_Text／Target_condition／Focus を代用してはならない。",
                ...
                "【出力上の禁止事項（方向性関連の補強）】",
                "- 出力文において、Org_Text／Target_condition／Focus に含まれる方向性語を根拠として引用・再掲してはならない。",
                "- 方向性の根拠は、Reference により特定した Result の値（符号・語句）のみに限定する。",
                "- なお、出力文の「➡ の前」は必ず『文章（Input）』の該当部分のみを引用すること。Org_Text（原文）や Focus の文言は引用しない。",
                ...
                "② Focus：Org_Textの中で特に注目すべき語句または数値。複数のチェックルールが同一のOrg_Textを持つ場合でも、Focusが異なれば注目点を変えて比較を行ってください。つまり、Focusが指す部分を重点的に評価対象とします。Focus に『プラス要因』『マイナス要因』『上昇』『下落』などの方向性語が含まれる場合は、数値の大小ではなく、方向の一致／不一致を最優先で判定してください。",
                ...
                "【出力ルール（簡潔版）】",
                "- Reference と Result から得られた結果が、Input 文章中の対応部分と意味・数値・方向性いずれの点でも完全に一致している場合は、出力は '一致' のみを返してください。",
                "- 不一致、または部分的に不整合な場合のみ、次の形式で理由を出力してください:",
                "  [{'original': '（Input文中の問題箇所）', 'reason': '（不一致の理由または補足説明）'}]",
                "- 一致の場合は理由説明を出してはいけません。必ず '一致' のみを返してください。",
                "- 判定不能の場合（比較対象が存在しない、Referenceで特定不可など）は '判定不能' と返してください。",

                "【最終目的】",
                "ReferenceとResultを用いて正確な数値または語句を導出し、それをInput内の意味・方向性と照らし合わせて整合性を判定します。",
                "結果が完全に一致していれば '一致'、そうでなければ不一致理由を出力してください。",
