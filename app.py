    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "【月次テーブル解釈ルール（厳守）】",
                "- 「|」で区切られた文字列はテーブルとして扱い、先頭行をヘッダー、先頭列を行名とみなし、特定の行名と列名の交点セルの値のみを抽出する。",
                "- Result が表形式の場合、列は各月、行は指標/効果種別とみなす（例：月次騰落率、為替効果、債券効果、株式効果、REIT効果など）。",
                "- Reference に『〜の行の〜の欄を参照します』等がある場合、そのセルの値のみを使用して置換を行う。",
                "- 年度累積などの集計列は比較対象外。",
