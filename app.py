    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "【初期実行命令（絶対遵守・省略禁止）】",
                "この処理は最初に必ず実行されます。省略・スキップ・順序入替は禁止です。",
                "1️⃣ Org_Text と Input の文頭を確認し、次の定型前置句が存在する場合は即時削除してください。",
                "   対象例：",
                "   「上記の環境のもと、」「上記の投資環境のもと、」「上記の環境下において、」「このような環境のもと、」「当月の市場環境を踏まえ、」「記の投資環境のもと、」「上記の投資環境を背景に、」など。",
                "   ※ 全角・半角空白や句読点の有無に関わらず削除してください。",
                "2️⃣ 削除後のテキストをそれぞれ『処理後 Org_Text』『処理後 Input』として定義します。",
                "3️⃣ 以降のすべての比較・抽出・判定処理では、必ず『処理後 Org_Text』『処理後 Input』のみを使用してください。",
                "4️⃣ 削除後の文頭に上記前置句が残っている場合、または削除後のテキストが空または8文字未満の場合、直ちに空文字（\"\")を出力し、以降のすべての処理を中止してください。",
                "5️⃣ この条件で出力された空文字は最終出力として確定し、他のルールをすべて無視します。JSON出力処理も実行してはいけません。",

                "【削除確認工程（中止条件付き）】",
                "削除後のテキストの文頭に前置句が残存していないか再確認してください。",
                "残存が検出された場合は、即座に空文字（\"\")のみを出力し、以降の全処理を停止します。",

                "【前提条件：原文一致性チェック（処理後テキストで実施）】",
                "処理後 Org_Text と 処理後 Input を比較し、文意・語彙の主要部分（主語・要因・述語）を照合します。",
                "主要名詞が一致しない場合、または Org_Text に存在しない名詞（米国債、株式、為替、金利、物価、セクター、国名、企業名など）が Input に含まれる場合、直ちに空文字（\"\")を返し、以降の判定を行わないでください。",
                "文型が類似していても、主語または要因の名詞が異なる場合は一致とみなしてはいけません。",
                "曖昧な場合も不一致と見なしてください。",
                ...
                "【入力定義】",
                "Org_Text = 「原文」セクションの内容、Input = 「文章」セクションの内容。",
