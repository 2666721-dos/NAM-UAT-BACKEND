    ...
    try:
        ...
        if not input:
            ...
            dt = [
                    ...
                    "出力は以下のJSON形式でお願いします:",
                    "- [{'original': '[原文中の誤っている部分:]', 'reason': '[理由:]'}]",
                    "- 類似したものがない場合は空文字列（\"\"）を返してください。",
                    "- 出力文には「Input」「Target_condition」「Result」「Origin_Text」「Focus」「Reference」という単語を含めないでください。",
                    "- 理由説明ではラベル名を使わず、内容だけで説明してください。",

                    "【定義】",
                    "- 「Input」：実際のレポート本文（複数段落を含む）。",
                    "- 「Origin_Text」：過去の参考情報であり、現行判定には使用しない。",
                    "- 「Result」：最終的な比較対象。JSON中の 'result' キーに指定された内容のみを比較対象とする。",
                    "- 「Target_condition」：補助的な判定基準（例：90%以上なら「高位」など）。",
                    "- 「Focus」「Reference」：参考情報。最終判定の根拠には使用しない。",

                    "【判定ルール】",
                    "1. 「Input」と「Result」の一致を最優先とする。",
                    "2. 両者が一致していれば正常（空文字列）とする。",
                    "3. 不一致の場合のみ誤りとして出力する。",
                    "4. 「Origin_Text」や「Target_condition」の内容は参考扱いとし、最終判定には使わない。",
                    "5. 「Result」に複数行（例：株式効果、債券効果など）がある場合、「Input」の文意に対応する行のみを比較対象とする。",
                    "6. 「Input」で示される資産クラス（株式・債券・通貨など）に対応する行を選択すること。",
                    "7. 「Focus」は補助情報であり、誤り判定の根拠にしてはいけない。",
                    "8. 「Focus」に「プラス」「マイナス」等が含まれていても、それは参考であり、判定基準には含めない。",
                    "9. 「Result」が意味を持たない文字列（例：「11111」「N/A」「-」「--」「該当なし」など）や、Inputと関連が見られない場合でも、それは不一致とみなし誤りとして出力する。",
                    "10. 「Result」に文章・説明・数値のいずれも含まれず、Inputの主題（資産・要因・動向など）と対応しない場合は、誤りとする。",

                    "【判断基準】",
                    "- 「Input」と「Result」の数値や表現が一致すれば正しい。",
                    "- 不一致の場合のみ誤りとして抽出する。",

                    "以下の要約文（Input）をもとに、「Result」との整合性を確認し、必要に応じて類似した原文を抽出してください。"
                ]
