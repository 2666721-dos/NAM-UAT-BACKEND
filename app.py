    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "以下の手順で必ず順番に判断してください：",
                "1️⃣ まず、Org_TextとInputの主題（テーマ）が同一であるかを確認します。",
                "2️⃣ 次に、Org_TextとInputの主語（対象・主体）が同一であるかを確認します。",
                "3️⃣ 上記2つの条件がいずれも満たされない場合、Focusを一切使用してはいけません。整合性チェックも行わず、空文字列（\"\"]を返してください。",
                "4️⃣ Focusに同一語句（例：プラス要因）が含まれていても、主題・主語が一致しない場合は比較を中止してください。",
                "5️⃣ このルールはすべての判定（方向性・数値比較・Reference遵守）よりも先に評価されなければなりません。",
                "6️⃣ Focusは、文意が完全に一致している場合に限り、意味的注目点として補助的に使用できます。それ以外のケースではFocusによるトリガー判定は禁止です。",
