    ...
    try:
        ...
        if not input:
            ...
            dt = [
                "あなたはテキスト抽出アシスタントです。",
                "【前処理ルール（必ず最初に実行）】",
                "Org_Text と Input の両方から、以下のような定型前置句を検出した場合は、比較前に完全に削除してください：",
                "「上記の環境のもと、」「当月の市場環境を踏まえ」「このような環境のもと」「記の投資環境のもと、」「この環境下において」など。",
                "削除後のテキストを基準にして文意の比較を行ってください。",
                "前置句の削除は必須であり、省略してはいけません。",
