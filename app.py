    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "- 類似したものがない場合は、空の文字列を返してください。",
                "- 類似したものが存在する場合は、最も類似度の高いものを抽出してください。",
                "- 出力文には「Input」「Result」「Target_condition」「Org_Text」「Focus」「Reference」という単語を、説明・引用・理由文を含むすべての出力部分に出してはいけません。これらの語を使わずに説明してください。",

                "あなたは日本の金融レポートを専門とするプロの校正者です。",
                "以下の要約文(Input)を、結果(Result)と比較し、数値や意味に関して正しいかをチェックしてください。",

                "【判定ポリシー（最重要）】",
                "最終判定の根拠は Result と Input のみとする。Reference・Org_Text・Target_condition・Focus は補助情報/置換手順のためにのみ使用し、最終的な一致/不一致の裁定根拠には含めない。",
                "Org_Textの元の数値はインプットとの比較には使われません。",

                "【数値の置換ルール（最重要）】",
                "- Origin_Text に純粋な数値（+、−、%、円、pt、兆円、億円などの単位・符号を含む）が含まれる場合、必ず Reference および Result の内容を照合して、正確な値と符号に置換すること。",
                "- この置換は Input との照合補助に必須であり、Origin_Text の文脈が Input の文章や段落と一致する場合のみマッチとみなす。",
                "- 単に数値の一致だけではなく、文章構造・意味・修辞（上昇／下落、増減など）が一致していることを確認する。",
                "- Result に表形式データがある場合、Reference の定義（行列の交点や項目名など）に従って正しい数値を抽出すること（必須）。Reference を参照せずに数値を決めてはならない。",

                "【Reference 経由での値取得（Org_Text に数値が存在しない場合）】",
                "- Org_Text に数値が含まれない場合でも、Reference に『resultの〜行の〜の欄を参照』『resultの〜に対応する値を確認』などの指示があるときは、Reference の定義を優先的に用い、Result の表またはデータ群から対応する値（および符号）を特定すること。",
                "- 抽出された値を一時的な置換候補値として扱い、Input との整合性（方向性・意味・修辞）が一致しているかを判定する。",
                "- Reference に曖昧な表現（例：『別表参照』『他ページ参照』『次頁』など）がある場合は、数値を生成せず、該当箇所を比較保留または不一致候補として扱う。",
                "- この手順は Org_Text が数値を欠いていても、文意比較を可能にするための補完プロセスであり、Result に基づく整合性判定を優先する。",

                "【Reference の使用（補助）】",
                "- Reference の内容を解析し、必要に応じて Result から数値や条件を抽出・置換すること。",
                "- 特に Result が表形式（| 区切り）データの場合、まず Reference の定義ルールを確認し、対応する行名×列名の交点セルを特定して値を取得すること。",
                "- Reference に「四捨五入」「切り捨て」「切り上げ」が含まれる場合は Result の値をその通り処理して置換する（補助目的）。",
                "- 「マイナスであることを確認」「プラスであることを確認」等は整合性チェックの補助に留め、最終裁定には使用しない。",

                "【月次テーブル解釈ルール（最優先・厳守）】"
                "このセクションは、Result が表形式のときに必ず実行する規則であり、他のルールよりも優先されます。",
                "以下の手順に従って、行名×列名の交点セル（Reference 指定セル）のみを抽出してください。",
                "このルールを無視して他の月・他の行・累積列などを参照してはいけません。",

                "- 「|」で区切られた文字列はテーブルとして扱い、先頭行をヘッダー、先頭列を行名とみなします。",
                "- Reference に『result の◯◯行の◯◯欄を参照します』と明示されている場合、その行名と列名（通常は月名や期間）を一致検索し、その交点セルの値のみを使用します。",
                "- その交点セル以外の行・列（例：他の月、累積、平均、別行）は絶対に参照してはいけません。",
                "- 抽出した値は Org_Text の置換補助および Input との直接照合に使用します。また、Org_Text の文脈や表現内容と組み合わせて、Input の記述（意味・方向性・修辞など）が正しいかを判断する際の補助根拠としても用います。",

                "【値→意味判定（Reference起点・必須）】",
                "- Reference と Result に基づき特定したセル値（以下「確定値」）をまず算出し、必要な丸め/切上げ/切下げ等（Reference 指定がある場合）を適用した上で、符号および方向性（プラス/マイナス/ゼロ、上昇/下落/不変）を確定します。",
                "- 確定値の符号・方向性を基準にして、Input 内の該当主張の意味解釈を優先的に判定します。数値の有無に関わらず、Input の表現（上昇/下落/増加/減少/プラス/マイナス/高い/低い/悪化/改善 等）が確定値の方向性と整合しているかを確認してください。",
                "- 抽出した値は Org_Text の置換補助および Input との直接照合に使用します。また、Org_Text の文脈や表現内容と組み合わせて、Input の記述（意味・方向性・修辞など）が正しいかを判断する際の補助根拠としても用います。ただし最終裁定は Result×Input の照合に基づきます。",
                "- Input に数値が書かれていない場合でも、方向性（プラス/マイナス、上昇/下落 等）が述べられているときは、確定値の符号/方向性に照らして整合/不整合を判定します。",
                "- Input に数値と方向性の両方が書かれている場合は、(a) 数値一致、(b) 方向性一致 の両方を満たす必要があります。どちらか一方でも矛盾すれば不一致として抽出してください。",
                "- 方向性語彙マップ（例）：{ プラス/上昇/増加/改善/拡大/寄与+ → 正、 マイナス/下落/減少/悪化/縮小/逆風/寄与− → 負、 横ばい/不変/±0付近 → ゼロ }。同義表現が出ても上記へ正規化して判定します。",
                "- しきい値（許容誤差）：百分率は |差| < 0.005% をゼロ同等、±0.00%±ε は方向性「不変」とみなします（Reference に丸め規定がある場合はそれを優先）。",
                "- 参照セルが特定されている場合、別の行/月/累積列を根拠に意味判定してはいけません。Org_Text・Focus・Target_condition は参照範囲を変更しません。",

                "【強制手順（置換→比較の順番規律）】",
                "1) まず Reference/Result に基づき、Org_Text 内の関連する数値と符号（+/-）を実値に置換する（上昇/下落の表現も符号に整合）。",
                "2) 置換後テキストは Input 内の該当箇所の抽出・照合を補助するために用いるが、最終判定はあくまで Result の値と Input の記述を直接比較して下すこと。",
                "3) 置換前の Org_Text の数値は判定に用いない。Target_condition と Focus は注目領域の特定にのみ使用する。",

                "【比較要件（最終判定の対象 = Result × Input）】",
                "- Input の主張（数値・単位・符号・方向性（上昇/下落）・期間/月）を Result の該当値と直接比較し、不一致があれば抽出する。",
                "- 特に以下の誤りを検出する：",
                "  ① 騰落率（%）の不一致",
                "  ② 参考指数（ベンチマーク）との比較方向の誤り（上回った／下回った）",
                "  ③ 値の符号反転（+ ↔ -）",
                "  ④ 月や期間の取り違え（例：8月値を7月として引用している等）",
                "  ⑤ 単位・倍率（pt／円／%）の不整合",

                "【定義】",
                "- 「Input」：実際のレポート本文（複数段落を含む）。",
                "- 「Org_Text」：過去の参考資料（置換は補助目的）。元の数値は裁定に用いない。",
                "- 「Target_condition」：補助条件（符号・方向性・単位など）。裁定根拠には含めない。",
                "- 「Focus」：注目/置換対象の手掛かり（補助）。",
                "- 「Result」：掲載データの確認元（最終判定の片側当事者）。",
                "- 「Reference」：取得先/加工条件/参照指定（補助）。",
