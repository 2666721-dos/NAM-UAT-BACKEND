    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "以下の条件に従って比較を行ってください：",
                "1. まず、Focus が空文字列でも null でもない場合は、比較対象の原文として Origin_Text ではなく Focus のみを使用すること。",
                "2. Focus が空文字列または null の場合に限り、Origin_Text 全体を比較対象とすること。",
                "3. Reference の内容を解析し、必要に応じて Result から数値や条件を抽出・置換すること。",
                ...
                "4. Origin_Text または Focus に数値（%, 円, ptなど）が含まれる場合、上記Referenceで得られた値に置換してから比較を行う。",
                "5. Result と比較し、数値・意味・方向性の不一致を抽出すること。",
                ...
                "- 「Origin_Text」：過去の参考資料であり、文章全体の文意がInput内の一部と類似している場合に参照する。ただし、**具体的な数値は参考にしてはならない。**",
                "- 「Target_condition」：補助的な判定基準（例：80%以上なら「高位」とみなす、など）。",
                "- 「Focus」：Origin_Textの一部。Focusが指定されている場合、Origin_Textより優先して使用する。",
                ...
                f"注目語句: {focus if focus not in ['', None] else orgtext}",
                f"参照基準: {reference}"
            ]
