    ...
    try:
        ...
        if not input:
            ...
            dt = [
                    ...
                    "- 「Input」は文章全体です。複数段落を含むことがあります。",
                    "- 「Origin_Text」はInput内の原文（文または段落）で、特定の表現が正しいかを確認する対象です。",
                    "- 「Result」は最終的な比較・検証対象であり、実際の数値・記述が含まれます。",
                    "- 「Result」と「Origin_Text」に含まれる数値や表現が異なる場合は、必ず「Result」を基準として判断してください。原文（Origin_Text）の値は参考情報であり、最終的な整合性判断には使用しないでください。",
                    "- 「Target_condition」はルールの判定基準を記述した補助情報です（例：90%以上であれば「高位」とする）。",
                    "- 「Target_condition」は参考として活用し、最終的な判定は「Result」と「Input」中の該当箇所との整合性によって行ってください。",
                    "- つまり、「Result」に含まれる数値や語句が、「Input（文章）」中の記載と一致していれば、それを正しいと判断してください。",
                    "- 「Origin_Text」や「Target_condition」と一致していなくても、「Result」と一致していれば問題ありません。",

                    "- 「Input」と「Result」に記載された数値や表現が一致する場合は、原文（Origin_Text）との相違があっても誤りとは判定しません。",
                    "- この場合は正常とみなし、出力を空文字列（\"\"）としてください。",

                    "- 「Focus」は「Origin_Text」内で特に注目すべき語句やフレーズです。",
                    "- 「Focus」は、特定の評価語（例：「高位」「プラス要因」「減少」など）であることが多く、それが文章中のどこに現れているかに注目してください。",
                    "- 「Focus」が短い場合でも、必ず「Origin_Text」内の語句との関連性を重視し、文章全体の中で誤って別の文脈に一致させないでください。",
                    "- 「Focus」に一致する語句が文章中に存在しても、それが「Origin_Text」全体の意味と一致しない場合、そのテキストは抽出しないでください。",

                    "- 「Reference」は補足的な注釈情報です。直接照合するものではありませんが、判断時の量的な基準理解の助けになります。",
