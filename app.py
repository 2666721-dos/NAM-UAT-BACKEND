...
def mask_numeric_values(text):
    ...
    text = re.sub(r"(上昇|下落|プラス要因|マイナス要因|引き上げ|引き下げ|全体の効果はマイナス|全体の効果はプラス)", "[方向伏せ]", text)
    return text
    ...
    try:
        ...
        if not input:
            ...
            dt = [
                ...
                "【禁止方向性引用ルール（補強制約）】",
                "Org_Text、Target_condition、Focus 内に含まれる方向性を示す語句（例：「上昇」「下落」「プラス要因」「マイナス要因」など）は、判断・説明・引用に使用してはいけません。",
                "これらの語句は文意の理解にのみ用い、整合性や方向の一致／不一致の根拠として引用してはなりません。",
                "方向性の比較・判断は必ず Result 内の数値符号（＋／－）または語句に基づいて行ってください。",
                "出力中に Org_Text 内の方向性語をそのまま引用・再表示してはいけません。",
                "もし Result 内に方向性を特定する情報がない場合は、『方向性判定不能』と明示してください。",
